...
function makeClock() {
    ...
    for (let tm = 0; tm < amountPointMinAll; tm++) {
        ...
        if (tm % amountPointM == 0) { radiusPoint = 5; } else { radiusPoint = 2; } //для выделения часовых рисочек
        let xPointM = xCenterClock + radiusNum * Math.cos(-divisorMinutes * tm * (pi / 180) + pi / 2); // расчет кординат рисок по формуле получения декартовых координат из полярных
        let yPointM = yCenterClock - radiusNum * Math.sin(-divisorMinutes * tm * (pi / 180) + pi / 2);
    ...
    for (let th = 1; th <= amountPointThAll; th++) {
        ...
        let xText = xCenterClock + (radiusNum - 30) * Math.cos(-divisorHours * th * (pi / 180) + pi / 2);// расчет кординат рисок по формуле получения декартовых координат из полярных
        let yText = yCenterClock - (radiusNum - 30) * Math.sin(-divisorHours * th * (pi / 180) + pi / 2);
    ...
    ctx.moveTo(xCenterClock, yCenterClock);
    ctx.lineTo(xCenterClock + lengthSeconds * Math.cos(pi / 2 - t_sec * (pi / 180)),
    ...
    ctx.lineWidth = 3;
    ctx.moveTo(xCenterClock, yCenterClock);
    ctx.lineTo(xCenterClock + lengthMinutes * Math.cos(pi / 2 - t_min * (pi / 180)),
    ...
    ctx.lineWidth = 5;
    ctx.moveTo(xCenterClock, yCenterClock);
    ctx.lineTo(xCenterClock + lengthHour * Math.cos(pi / 2 - t_hour * (pi / 180)),
...
let timerClock = window.setInterval(() => { makeClock(); }, 00);
...
// ===========================================================================================================
function getParameters(inputYear, forMonth) {//Футкция которая получает требуемый год и месяц и возвращает параметры этого года: високосные или нет, какой первый день месяца, год, кол-во дней в месяце, массив дней недели,название месяца  
    ...
    if (forMonth > 2) {                     /*т.к в високосные годы из за дополнительного дня в феврале начало месецев смещается на один день начиная с марта а январь и февраль 
    ...
    for (let i = 1972; i <= inputYear; i++) {     //цикл переберает годы с 1972 по заданный нами год

        if (iterLeapYears % 4 == 0) {           //iterLeapYears инкриментируется  ниже в коде и когда она кратна 4 условие верно это соответствеут весокосному году
            ...
            if (forMonth > 2) {                             //второй блок описание выше
                dayStart += 1;
        ...
        if ((iterLeapYears - 1) % 4 == 0) {     //нижнии два блока иф вычесляют с какого дня будет начинатся год это поможет при отрисовки календаря
            if (forMonth > 2) {                   //третий блок описание выше
                dayStart++;
    ...
    if (leapYearsBull && forMonth == 2) { amountDays = 29 }   //если год високосный и месяц февраль мы задаем число дней 29
...
function getFanParameters(inputYear, forMonth) {//Футкция которая получает требуемый год и месяц и возвращает параметры этого года: високосные или нет, какой первый день месяца, год, кол-во дней в месяце, массив дней недели,название месяца 

    let arrFanDays = ['Me', 'Ve', 'Ti', 'Ma', 'Jo', 'Sa', 'Ur', 'Ne', 'Pl'];
    let arrFanMonth = ['', 'Einnum', 'Tveyrus ', 'Trirum ', 'Fhorirus ', 'Fimmum ', 'Sextum ', 'Scheus ', 'Attum ', 'Nius', 'Tium ', 'Ellefum ', 'Tolfus ', 'Terettanum ', 'Fhortanus  ', 'Fimmtanum ', 'Sextanums'];
    let arrFanStartDays = [0, 1, 9, 9, 8, 8, 7, 6, 6, 5, 5, 4, 3, 3, 2, 2, 3];
    let arrFanAmountDays = [0, 26, 27, 26, 27, 26, 26, 27, 26, 27, 26, 26, 27, 26, 27, 26, 26];
    ...
    for (let i = 1972; i <= inputYear; i++) {     //цикл переберает годы с 1972 по заданный нами год
        ...
        if (iterLeapYears % 2 == 0) {           //iterLeapYears инкриментируется  ниже в коде и когда она кратна 2 условие верно это соответствеут весокосному году
        ...
        if ((iterLeapYears - 1) % 2 == 0) {     //нижнии два блока иф вычесляют с какого дня будет начинатся месяц это поможет при отрисовки календаря
    ...
    if (leapYearsBull && forMonth == 16) { amountDays = 27 } //если високосный год и 16 месяц то кол-во дней 27
...
//  ****************************************************************************** getFanParameters ***************************************************************
// ************************************************************************************************
function drowCalendar(amountDays, arrDays, startMonth, monthName, year = '2025') { //рисует календарь входные параметры: кол-во дней в месяце, массив дней ,день начала месяца,имя месяца, текущий год
    ...
    for (let i = 0; i < arrDays.length; i++) { //циклом собираем ячейки таблици с названиями дней в ряд
    ...
    while (days <= amountDays) { //здесь тагже создаются ячейки таблици только уже с числами 
        ...
        while (widthWeek > 0) {
            ...
            widthWeek--;
            tr.appendChild(td);
            if (days > amountDays) { break } //когда все дни прописаня прекращает цикл
        }
        table.appendChild(tr); //кладем ряд таблици в таблицу
...
function startCalendar(year, month, transit) { //эта функция отрисовывает наш календарь  transit - используется для переключения между временами
    //вызов функций для получения параметров  если transit === 16 то вызываем getFanParameters иначе getParameters
    if (transit === 16) { arrParameters = getFanParameters(year, month); } else { arrParameters = getParameters(year, month); }
    //рисуем календарь
    drowCalendar(arrParameters.amountDays, arrParameters.arrDays, arrParameters.dayStart, arrParameters.monthName, arrParameters.year);
...
function chengTime(currentYear, currentMonth, today, nowDate, transit) { //функция перелистования годов и месяцев
    ...
    for (let i = 1975; i < 2036; i++) {                //отрисовка option для select
    ...
    function monthLess() { // ***************************************************************Less
        ...
        if (currentMonth <= 1) { currentMonth = transit - 1; currentYear--; }
...
function choiceToday(currentYear, currentMonth, today, nowDate) {
    ...
    if (currentYear == nowDate.getFullYear() && currentMonth == (nowDate.getMonth() + 1)) { //проверка месяца и года что-бы не отрисовывалось на всех месяцах
