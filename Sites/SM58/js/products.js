class Producrs {
  constructor(gallery, basket, productsCatalog) {
    //2 получил класс galery and arr productsCatolog
  ...
  create() {
    ...
    for (var i = 0; i < this.productsCatalog.length; i++) {
      ...
      var img = serviseCreaetElement.getElement({
        ...
        click: `testclick(${this.productsCatalog[i].id})`,
        className: "img",
        backgroundImage: `url(${this.productsCatalog[i].img})`,
        innerText: "",
      }); //вызвал метод и передал  объект
      var size = serviseCreaetElement.getElement({
        tagName: "div",
        className: "size",
        innerText: "Размер: " + this.productsCatalog[i].size,
      });
      var btn = serviseCreaetElement.getElement({
        tagName: "button",
      ...
      var model = serviseCreaetElement.getElement({
        tagName: "div",
        className: "model",
        innerText: "Модель: " + this.productsCatalog[i].id,
      });
